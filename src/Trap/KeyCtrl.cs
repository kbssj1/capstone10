using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class KeyCtrl : MonoBehaviour, IListener {

    private Survivor survivor;
    private bool possible;

    public GameObject Object;


    public GameObject Elevator;
    public GameObject Ele_Door;



    // Use this for initialization
    void Start () {
        possible = false;
      
        EventManager.Instance.AddListener(EVENT_TYPE.SURVIVOR_CREATE, this);
        EventManager.Instance.AddListener(EVENT_TYPE.KEY_SUCCESS, this);
    }
	
    IEnumerator CheckKeyCtrl() // 
    {
        yield return null;

        while (true) {
            
            Vector3 viewPos = Camera.main.WorldToViewportPoint(Object.GetComponent<Transform>().position); // 카메라 뷰포트로 변환
                                                                                                                 
            if(IsAbleKeyOpen(viewPos))
            {
                KeyOpen();               
            }

            yield return null;
        }
        
    }
    bool IsAbleKeyOpen(Vector3 viewPos)
    {
        return possible && viewPos.x > 0.1f && viewPos.x < 0.9f && viewPos.y > 0.1f && viewPos.y < 0.9f && Input.GetButtonDown("Fire2") && survivor.getItemKey();
    }

    void OnTriggerEnter(Collider col)
    {
        if (col.CompareTag("SURVIVOR") && survivor.getItemKey())
        {
            possible = true;
            EventManager.Instance.PostNotification(EVENT_TYPE.B_RIGHT_BTN_POSSIBLE, this);
        }
    }

    void OnTriggerExit(Collider col)
    {
        if (col.CompareTag("SURVIVOR") && survivor.getItemKey())
        {
            possible = false;
            EventManager.Instance.PostNotification(EVENT_TYPE.B_RIGHT_BTN_IMPOSSIBLE, this);
        }
    }


    public void KeyOpen()
    {
        
        survivor.GetComponent<KeyTrap1>().enabled = true;

        
        EventManager.Instance.PostNotification(EVENT_TYPE.SURVIVOR_KEYCTRL, this);
    }

    
    public IEnumerator OpenDoor()
    {
        Elevator.SetActive(true);
        yield return new WaitForSeconds(8.0f);
        Ele_Door.GetComponent<Animator>().SetBool("RightOpen", true);
    }

    public void OnEvent(EVENT_TYPE Event_Type, Component Sender, object Param)
    {

        switch (Event_Type)
        {

            case EVENT_TYPE.SURVIVOR_CREATE:

                survivor = GameObject.FindGameObjectWithTag("SURVIVOR").GetComponent<Survivor>();

                if(survivor.GetPhotonView().isMine)
                {
                    StartCoroutine(CheckKeyCtrl()); //코루틴 실행

                }

                break;

            case EVENT_TYPE.KEY_SUCCESS:

                StartCoroutine(OpenDoor());
                
                break;
        };
    }

}
